# Case AI Snapshot Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a collapsible "🤖 AI 案件简报" panel to the case detail modal that generates an operational case brief from Google Drive + Gmail, and applies extracted data (status, timeline, docs, key issues, next steps) back to the case record.

**Architecture:** New `src/CaseAI.jsx` component reads the OAuth session internally (no prop needed), fetches Drive/Gmail data in parallel, calls Claude to generate a 6-section bilingual brief, then on Apply does a second Claude call to extract structured JSON and merge it into the case. Wired into `src/App.js` at the case detail modal (`viewJob`) inside the `Jobs` component.

**Tech Stack:** React 18, Anthropic Claude API (`/api/claude`), Google Drive API v3 (`/api/drive-sync`), Gmail API (`/api/gmail-sync`), mammoth (DOCX text extraction), Vercel serverless
...
|------|--------|----------------|
| `src/CaseAI.jsx` | **Create** | Complete case brief UI + Drive/Gmail fetch + Claude calls + merge logic |
| `src/App.js` | **Modify** | Import CaseAI; add `handleSaveCase` inside `Jobs`; render `<CaseAI>` in viewJob modal |

---

## Task 1: Create `src/CaseAI.jsx`

**Files:**
- Create: `src/CaseAI.jsx`

### Context before writing

The `Jobs` component in `App.js` (line 3228) owns `const [viewJob, setViewJob] = useState(null)` (line 3238). The case detail modal is the `{viewJob && (() => { ... })()}` block (lines 3497–4092). CaseAI will be rendered inside that modal.

OAuth session is stored in `sessionStorage` under key `ozsky_gmail_session`. `readSession()`, `sessionIsValid()`, and `getValidToken()` from `src/utils/gmailSession.js` handle all token logic — no `gmail` prop needed.

`/api/drive-sync` accepts `{ accessToken, clientName }` and returns `{ folderFound, folderName, totalFiles, processed: [{ id, name, mimeType, textContent, base64Content, skipped, error }] }`.

`/api/gmail-sync` accepts `{ accessToken, maxResults, q }` and returns `{ emails: [{ date, subject, from, ai: { rawSummary, keyNeeds, isRelevant } }] }`.

`/api/claude` accepts standard Anthropic messages API body plus optional `_beta` field for PDF support. Returns non-JSON on 413 (body too large).

- [ ] **Step 1: Write `src/CaseAI.jsx`**
...
简报文本：\n${brief}`,
      ...
      // Balanced-bracket JSON extractor (same as SmartAI handleApply)
      const jsonStr = (() => {
      ...
      for (const [k, v] of Object.entries(ex.docs || {})) {
        if (!(k in mergedDocs)) {
          mergedDocs[k] = v;           // new key — add as-is
        } else if (v === true) {
          mergedDocs[k] = true;        // upgrade to received
        }
...
If build fails, fix syntax errors in `src/CaseAI.jsx` before continuing.

- [ ] **Step 4: Commit**

```bash
git add src/CaseAI.jsx
git commit -m "feat: add CaseAI component for case-specific AI brief generation"
```

---

## Task 2: Wire `CaseAI` into `App.js`

**Files:**
- Modify: `src/App.js` (3 changes)

### Change 1 — Import CaseAI
...
Add the CaseAI import directly after it.

- [ ] **Step 1: Add import**

In `src/App.js`, after line 4 (`import SmartAI from './SmartAI';`), insert:
```js
...
### Change 2 — Add `handleSaveCase` inside `Jobs`

The `Jobs` function starts at line 3228. `viewJob` state is declared at line 3238:
```js
const [viewJob, setViewJob] = useState(null);
```

Add `handleSaveCase` immediately after that line (after line 3239, the `clientSearch` state line). Both `setViewJob` and `setJobs` (a prop) are in scope here.

- [ ] **Step 2: Add `handleSaveCase` inside `Jobs`**

After this block in `src/App.js` (around line 3239):
```js
  const [viewJob, setViewJob] = useState(null);
...
### Change 3 — Render `<CaseAI>` in the viewJob modal

The viewJob modal body ends with a notes section (closes around line 4080) followed by the footer action bar (line 4082). Insert `<CaseAI>` between them.

The text to find (around line 4081–4082):
```jsx
      </div>
...
- [ ] **Step 3: Render CaseAI before the footer**

In `src/App.js`, find the block below (it appears once, near the end of the viewJob modal, around line 4081):
...
If build fails with "CaseAI is not defined" — check the import was added correctly in Change 1.
If build fails with "handleSaveCase is not defined" — check it was added inside the `Jobs` function (not outside it).

- [ ] **Step 5: Commit and push**

```bash
git add src/App.js
git commit -m "feat: wire CaseAI into Jobs case detail modal"
git push
```

Expected: push succeeds and Vercel auto-deploys.
...
- [ ] Open any case from the Jobs page → case detail modal opens
- [ ] "🤖 AI 案件简报" panel is visible at the bottom of the modal, collapsed by default
- [ ] Click the panel header → it expands showing "✨ 生成案件简报" button
- [ ] Click "✨ 生成案件简报" → step text shows "📁 读取 Drive 文件夹...", then "📧 读取相关邮件...", then "🤖 生成案件简报..."
- [ ] Brief text appears in scrollable textarea after generation
- [ ] "⬆️ 应用到案件" button appears after brief is generated
- [ ] Click "⬆️ 应用到案件" → shows "✅ 已应用到案件档案" for 4 seconds
- [ ] Close and reopen the case → notes contain "[AI 案件简报 YYYY-MM-DD]" entry
- [ ] If no Gmail session: Drive status shows "📁 未找到客户文件夹..." or equivalent; generation still proceeds from CRM data only
- [ ] Same case opens correctly from the client's "案件" tab
