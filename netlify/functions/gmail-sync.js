...
// Netlify Functions adapter for Gmail sync + AI analysis
...
exports.handler = async (event, context) => {
  ...
  if (!accessToken) {
    return { statusCode: 401, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ error: 'No Gmail access token provided' }) };
  }
  ...
  try {
    ...
    if (!listRes.ok) {
      ...
      return { statusCode: listRes.status, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ error: err.error?.message || 'Gmail API error' }) };
    }
    ...
    // ── 2. Fetch full content for each email ──────────────────────────────────
    ...
    const parsedTexts = rawEmails.map(email => {
      ...
      const bodyTrunc = emailBody.slice(0, 3000);
      return {
        messageId: email.id,
...
请返回**纯 JSON**（不含 markdown 代码块），格式如下：
{
  ...
  "rawSummary": "邮件内容的1句话摘要（中文）"
    ...
    return {
      ...
      body: JSON.stringify({ emails: sorted, total: sorted.length }),
    };
  } catch (err) {
